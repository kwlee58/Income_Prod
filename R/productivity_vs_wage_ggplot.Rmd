...
text.lab <- paste(format(end.df$Cumulative.Changes, digits = 1, nsmall = 1), "%", sep ="")
(g1 <- ggplot() +
  geom_line(data = prod.wage.melt, aes(x = Year, 
...
(g2 <- g1 + 
  geom_point(data = end.df, aes(x = Year, 
                                y = Cumulative.Changes, 
                                colour = Prod.or.Compensation), 
             size = 3, 
             show.legend = FALSE))
...
```{r, echo = TRUE, fig.width = 8, fig.height = 6}
(g3 <- g2 +
  geom_text(data = y1995.df, aes(x = Year, 
                                 y = Cumulative.Changes - c(10, 20), 
                                 label = var.lab)) +
  geom_text(data = end.df, aes(x = Year, 
...
(h1 <- ggplot() +
  geom_line(data = prod.wage.2.melt, aes(x = Year, 
                                         y = Cumulative.Changes, 
                                         colour = Prod.or.Compensation), 
            size = 1.5, 
            show.legend = FALSE))
...
```{r, echo = TRUE, fig.width = 8, fig.height = 6}
(h2 <- h1 + 
  geom_point(data = end.df.2, aes(x = Year, 
                                  y = Cumulative.Changes, 
                                  colour = Prod.or.Compensation), 
             size = 3, 
             show.legend = FALSE))
...
(h3 <- h2 +
  geom_text(data = y2007.df, aes(x = Year, 
                                 ...
                                 label = var.lab.2)) +
  geom_text(data = end.df.2, aes(x = Year + 2, 
                                 y = Cumulative.Changes, 
                                 label = paste(Cumulative.Changes, "%", sep = ""))))
```
